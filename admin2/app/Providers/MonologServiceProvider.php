...
class MonologServiceProvider implements ServiceProviderInterface, BootableProviderInterface, EventListenerProviderInterface
{
    public function register(Container $app)
    {
        ...
        $app['slack.logger.reporting'] = function ($app) {
            $logger = new Logger('slack');

            if (!empty($app['reporting']['slack-webhook'])) {
                $slackHandler = new SlackWebhookHandler(
                    $app['reporting']['slack-webhook'], // your webhook URL
                    null,       // channel (can be null)
                    'AlertBot',      // username
                    true,            // use attachment
                    ':warning:',     // icon
                    false,           // short attachment
                    false,           // include context/extra
                    Logger::WARNING  // minimum level
                );
